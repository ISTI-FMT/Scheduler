...
void tabellaFermate::leggifileconfigurazioneItinerari(String ^nomeFile)
{
	...
	while (reader->ReadToFollowing("fermata")){
		...
		while (reader->ReadToFollowing("binario"))
		{
			...
			System::String ^StrIdBinario = reader->GetAttribute("id");
			// converto id dellbinario da System::String a int
			int idBinario = convertiString2int(StrIdBinario);
			// setto id del binario
			bin->setId(idBinario);

			// ...leggo direzione del binario
			System::String ^direzione = reader->GetAttribute("direzione");
			// setto direzione del binario
			bin->setDirezione(direzione);

			// ...leggo nid_lrgb del binario
			System::String ^Strnid_lrgb = reader->GetAttribute("nid_lrgb");
			...
			System::String ^Strd_stop = reader->GetAttribute("d_stop");
			...
			System::String ^Strportebanchina = reader->GetAttribute("portebanchina");
			// converto in bool
			...
			System::String ^latobanchina = reader->GetAttribute("latobanchina");
			// setto latobanchina del binario
			bin->setLatoBanchina(latobanchina);

			// ...leggo nextCDB del binario
			System::String ^StrnextCDB = reader->GetAttribute("nextCDB");
			...
			System::String ^StrprevCDB = reader->GetAttribute("prevCDB");
			...
			System::String ^StrCDB = reader->ReadString();
